...
NameOverride nameOverrides[MAX_DISC_ENTITIES];
...
void applyNameOverrides() {
  for (int i = 0; i < discEntityCount; i++) {
    for (int j = 0; j < nameOverrideCount; j++) {
      if (strcmp(discEntities[i].entityId, nameOverrides[j].entityId) == 0) {
        strlcpy(discEntities[i].name, nameOverrides[j].displayName, sizeof(discEntities[i].name));
        break;
      }
    }
  }
